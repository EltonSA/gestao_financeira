...
import { ExpenseFormFields, type ExpenseFormCtx } from "@/components/forms/expense-form-fields";

export function NovaDespesaForm({ ctx }: { ctx: ExpenseFormCtx }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-7 shadow-[var(--shadow-xs)]">
        <ExpenseFormFields ctx={ctx} />
      </div>
      <div className="flex flex-col-reverse sm:flex-row gap-3">
        <Button asChild variant="ghost" className="flex-1">
          <Link href="/despesas">Cancelar</Link>
        </Button>
        <Button type="submit" loading={load} className="flex-1">
          {load ? "Salvando…" : "Salvar despesa"}
        </Button>
      </div>
    </form>
  );
