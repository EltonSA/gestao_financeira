...
import {
  ...
  type RecurringIncomeFormCtx,
  type RecurringIncomeFormDefaults,
} from "@/components/forms/recurring-income-form-fields";

export function EditarRecebimentoRecorrenteForm({
  id,
  initial,
  ctx,
}: {
  id: string;
  initial: RecurringIncomeFormDefaults;
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const r = await updateRecurringIncomeAction(id, fd);
    setLoad(false);
        ...
        <ConfirmDialog
          trigger={
            <Button
              ...
              className="text-[var(--danger)] sm:mr-auto"
            >
              Excluir modelo
            </Button>
          }
          title="Excluir este modelo?"
          description="O modelo será removido. Entradas geradas anteriormente são mantidas."
          confirmLabel="Excluir"
          onConfirm={onDelete}
        />
        <Button asChild type="button" variant="ghost">
          <Link href="/entradas">Voltar</Link>
        </Button>
